...
def AFNN(filters=8, image_channels=1, use_bnorm=True):
    ...
    for i in range(3):
        ...
        x_1 = Conv2D(filters=filters, kernel_size=(3, 3), strides=(2, 2),
                     kernel_initializer='Orthogonal', padding='same', use_bias=False,
                     name='conv_p1_'+str(layer_count))(x_1)
        if use_bnorm:
            layer_count += 1
            x_1 = BatchNormalization(
                axis=3, momentum=0.0, epsilon=0.0001, name='bn_p1_'+str(layer_count))(x_1)
        layer_count += 1
        x_1 = Activation('relu', name='relu_p1_'+str(layer_count))(x_1)
    ...
    # 4 layers, Conv+BN+ReLU
    layer_count += 1
    x_2 = Conv2D(filters=filters, kernel_size=(5, 5), strides=(2, 2),
                 kernel_initializer='Orthogonal', padding='same',
                 name='conv_p2_'+str(layer_count))(x_0)
    layer_count += 1
    ...
    for i in range(3):
        ...
        if use_bnorm:
            ...
            x_2 = BatchNormalization(
                axis=3, momentum=0.0, epsilon=0.0001, name="bn_p2" + str(layer_count))(x_2)
        layer_count += 1
        x_2 = Activation("relu", name="relu_p2_" + str(layer_count))(x_2)

    # Path 3
    # 2 layers, Conv+BN+ReLU
    layer_count += 1
    x_3 = Conv2D(filters=filters, kernel_size=(7, 7), strides=(4, 4),
                 kernel_initializer='Orthogonal', padding='same',
                 name='conv_p3_'+str(layer_count))(x_0)
    layer_count += 1
    x_3 = Activation('relu', name='relu_p3_'+str(layer_count))(x_3)
    layer_count += 1
    x_3 = Conv2D(filters=filters, kernel_size=(7, 7), strides=(4, 4),
                 kernel_initializer='Orthogonal', padding='same',
                 name='conv_p3_'+str(layer_count))(x_3)
    layer_count += 1
    x_3 = Activation('relu', name='relu_p3_'+str(layer_count))(x_3)
    if use_bnorm:
        layer_count += 1
    ...
    # Merge layer
    layer_count += 1
    x = Concatenate(axis=-1, name='concat'+str(layer_count))([x_1, x_2, x_3])
    layer_count += 1
...
if __name__ == '__main__':
    ...
    # compile the model
    AF_model.compile(optimizer=ad, loss=losses.mean_absolute_error)
